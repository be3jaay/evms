...
Class Action {
	...
	function save_book() {
		...
		$existing_booking = $this->db->query("SELECT * FROM venue_booking WHERE venue_id = '$venue_id' AND datetime = '$schedule' AND id != '$id'");
		if ($existing_booking->num_rows > 0) {
			// Schedule is already booked, return a specific response
			echo json_encode(array('response' => 'already_booked'));
		...
		echo json_encode(array('response' => $save ? 1 : 'Error saving book'));
		exit(); // Make sure to exit to prevent further execution
	}
